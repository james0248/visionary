"""Pixel-space teacher-forced probe with a moving-region mask.

Latent L2 near the jitter floor cannot distinguish "copied the background"
from "predicted everything predictable", so this probe scores in pixels
...
A model that copies scores at `copy`; a perfect dynamics model scores at
`recon`. Where tf sits between them is the honest answer. Writes per-frame
metrics JSON, a 4-panel video (raw | recon | tf | roll), and a contact-sheet
PNG of the frames where tf falls furthest below the tokenizer ceiling.

    uv run python scripts/analysis/eval_pixel_tf.py \
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Pixel-space teacher-forced probe.")
    ...
    parser.add_argument("--label", required=True, help="Tag prefixed to every output file.")
    ...
    for index in (int(i) for i in args.indices.split(",")):
        ...
        logger.info(
            "clip %d | moving-psnr  recon %.2f  tf %.2f  roll %.2f  copy %.2f",
            index,
            *(mean_of(entry["psnr_moving"][k]) for k in ("recon", "tf", "roll", "copy")),
        )

        if not args.no_video:
            panels = [raw, recon, tf_px, roll_px]
            ...
            imageio.mimsave(
                output_dir / f"{args.label}_{step}_{index:03d}_raw-recon-tf-roll.mp4",
                out_frames,
                fps=args.fps,
            )
        ...
        if worst:
            ...
            imageio.imwrite(output_dir / f"{args.label}_{step}_{index:03d}_worstframes.png", sheet)
    ...
    (output_dir / f"{args.label}_pixel_tf.json").write_text(json.dumps(out, indent=2))
    logger.info("Wrote %s", output_dir / f"{args.label}_pixel_tf.json")
