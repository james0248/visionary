...
**Tokenizer Dataset Inspection**

Use [tokenizer_dataset_inspect.yaml](/Users/hyeonseok/Documents/Sources/visionary/cloud/tokenizer_dataset_inspect.yaml) to launch a small `v6e-1` TPU VM that mounts `visionary-data-tokenizer-ue1d` read-only and scans the enduro, qbert, and seaquest tokenizer ArrayRecord datasets:

```bash
uv run python cloud/watcher.py --config cloud/tokenizer_dataset_inspect.yaml
```
