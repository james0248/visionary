...
- Retested the npm dev `onnxruntime-web@1.27.0-dev.20260506-673c3320fc` dist the same way against
...
- Accepted a split-only MHA GQA pretranspose rewrite for the derived WASM dynamics files. The pass
  rewrites each split K/V `Gather(axis=2) -> Transpose(0,2,1,3)` feeding
  `com.microsoft::MultiHeadAttention` into `Transpose(compact K/V) -> Gather(axis=1)`, moving the
  repeated-head expansion after the compact K/V transpose. Native ORT stayed bit-exact against the
  pre-rewrite split files (`max_abs 0.0` for `final_z`, `candidate_k_entry`, and
  `candidate_v_entry`). Chrome actual-demo validation passed at `37.84 fps` / `26.43 ms`, with
  dynamics `24.24 ms` (`sample 16.19 ms`, `entry 8.01 ms`) and cache wait `1.46 ms`.
  Safari-family/WebKit unsplit default validation still passed at `32.65 fps` / `30.63 ms` with
  `wasmNumThreads=3`. An explicit split retest then passed at `35.90 fps` / `27.86 ms`, so the demo
...
- Accepted a split-only head-time cache ABI rewrite for the WASM sample/entry dynamics graphs. The
  pass changes the split cache inputs from `[layer,batch,token,time,head,dim]` to
  `[layer,batch,token,head,time,dim]`, then rewrites the split MHA K/V cache paths so cache and
  current K/V concatenate directly in compact B,H,S,D layout before the GQA head gather. It removes
  `72` sample transposes (`2308 -> 2236` nodes) and `22` entry transposes while inserting `11`
  parallel entry V-current concats (`1178 -> 1167` nodes). Native ORT stayed bit-exact against the
  accepted split files when fed transposed K/V caches (`max_abs 0.0` for `final_z`,
  `candidate_k_entry`, and `candidate_v_entry`). The demo keeps the served initial-cache sidecars in
  the original layout and transposes them in memory only when the split WASM metadata requests the
  head-time cache layout, so the WebGPU/full-step ABI is not changed. Browser output and latent
  validation passed through the actual demo proxy: Chrome measured `39.92 fps` / `25.05 ms`, with
...
- Rejected lazily constructing the full-step feed object only on the non-split path in
  `generateFrame`. The split WASM path does not consume that feed object, but the Chrome actual-demo
...
- Accepted a browser-family decoder-worker retune and benchmark default cleanup. The actual demo now
  defaults the decoder worker to `3` threads on Chromium and `2` threads on non-Chromium browser
  profiles. The benchmark no longer forces `decoderWorkerNumThreads=3`, so the WASM benchmark is
  again a proxy for the demo defaults unless the wrapper flag explicitly overrides it. Chrome
  rejected decoder-worker `2` and `4` thread controls (`40.87 fps` and `40.63 fps`, both below the
  default/noisier `3`-thread windows). WebKit/Safari-family improved with a two-thread decoder
  worker: the explicit `2`-thread run measured `39.58 fps`, and the final default benchmark measured
...
- Revised the active performance target to `45 fps` for both Chrome and Safari/WebKit. Accepted a
  full-head-time WASM layout cleanup stack: identity `Gather(0..255 reshaped as 8x32)` nodes are now
  exported as `Reshape`, static `Split -> Unsqueeze -> Concat` head layouts are exported as
  `Reshape`/`Transpose`, packed QKV split outputs that were immediately rebuilt into two-head
  groups are coalesced before reshaping, and the existing rank-2 SwiGLU island rewrite is applied to
  the derived full-head-time graph. CPU ORT comparison of the generated graph against the trial graph
  was exact for `final_z`, `candidate_k_entry`, and `candidate_v_entry`.
- Accepted two runtime scheduling tweaks for the unlimited-FPS demo stream: zero-delay loops use
  `queueMicrotask` when available, and the full-step path starts the decoder worker immediately
  after dynamics returns instead of after cache-update setup. Default actual-demo validation passed
  in Chrome at `46.07 fps` (`19.80 ms` dynamics, `1.21 ms` cache, `22.78 ms` decoder total). The
  best short WebKit/Safari-family window reached `43.60 fps` (`21.18 ms` dynamics, `0.90 ms` cache,
  `23.88 ms` decoder total); the longer 128-frame WebKit validation settled at `42.06 fps` with
  visual, latent, and frame-stability validation still passing. Chrome now meets the revised target,
  but WebKit/Safari-family remains short.
- Rejected the following follow-up attempts on the optimized graph: preallocated WASM CPU output
  tensors failed validation by producing static zero latents; the async worker cache updater reduced
  decoder wait but regressed WebKit to `41.13 fps`; a row-contiguous K-cache update loop was exact
...
- Rejected several additional post-45fps probes on the current full-head-time WASM path:
  ...
  - Tried a gated two-decoder-worker pool to see whether parallel decoder throughput could beat the
    single worker's `23-25 ms` critical path. WebKit/Safari-family output and latent validation
    passed, but both shapes regressed: two one-thread decoder workers measured `37.53 fps` with
...
- Rejected adding static `value_info` shape metadata to custom-op outputs in the full-head-time
  dynamics ONNX. A local ignored-asset trial propagated shapes through simple ORT contrib ops and
  added `270` value-info entries, but native Safari timed out during the benchmark execute call
  instead of producing a faster window. The original ONNX asset was restored to hash
  `c1f8695bcab54217b7ba81af651f0091d13be0318c3afbd45c0143c5d707af17`.
- Rejected increasing the native Safari benchmark's excluded warmup window from `32` generated
  frames. After a restored accepted-path default sample missed the 45 fps gate at `43.80 fps` and
...
- Rejected packing the last remaining same-input dynamics `Gemm` pair into one wider `Gemm` plus
  `Split`. The ignored-asset trial was CPU-exact for `final_z`, `candidate_k_entry`, and
  `candidate_v_entry`, but Chrome stayed flat at `47.20 fps` and native Safari regressed to
  `44.76 fps`. Keep the two small tail `Gemm` nodes.
- Rejected stripping decoder `value_info` metadata from the base and Safari/WebKit MHA decoder
  artifacts. The trial was CPU-exact and browser-valid, but it was noise-level rather than a real
  speedup: Chrome measured `47.23 fps` and native Safari `45.45 fps`, both within the accepted
  variance envelope. Keep the generated decoder artifacts unchanged.
- Rejected a native Safari `wasmNumThreads=5` retune on the current full-head-time path. Validation
  still passed, but steady-state throughput fell to `32.15 fps`, so the default remains `3` main
  WASM threads and `2` decoder-worker threads for Safari-family browsers.
- Rejected preallocating only the WASM `final_z` output while leaving cache-entry outputs
...
- The current validated default windows are about `46-47 fps` in headed Chrome, about `39-44 fps`
  ...
  on this machine; native Safari can reach it but is not reliable enough yet, and Playwright WebKit
